...
**加载顺序（严格依赖）**：`i18n.js` → `storage.js` → `archive.js` → `markdown.js` → `tabs.js` → `editor.js` → `app.js`

### 模块职责

| 模块 | 全局名 | 职责 |
|------|--------|------|
| `storage.js` | `Storage` | IndexedDB 笔记 CRUD、旧数据迁移、设置管理、JSON 备份 |
| `archive.js` | `Archive` | Markdown ZIP 生成/解析、批量 Markdown 导入 |
...
```js
// IndexedDB: notej_db
// object store: notes（每篇笔记独立记录）
{ id: 'note_xxx', title: '标题', content: 'Markdown 文本', createdAt, updatedAt }
// object store: meta
{ key: 'workspace', activeTabId: 'note_xxx', tabOrder: ['note_xxx'] }
// localStorage key: 'notej_settings'
{ theme: 'auto'|'light'|'dark', fontSize, splitRatio, lastActiveTabId }
```

旧版 `localStorage.notej_data` 在首次启动时自动迁移到 IndexedDB，并保留到
`localStorage.notej_data_migrated_backup`。
