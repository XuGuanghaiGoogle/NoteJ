...
const I18n = (() => {
    ...
    /** 新版存储与导入导出界面的补充翻译 */
    const _storageTranslations = {
        'zh-CN': {
            'toolbar.backupJson': '导出完整 JSON 备份',
            'toolbar.exportMarkdown': '导出 Markdown ZIP',
            'toolbar.importNotes': '导入 JSON、ZIP 或 Markdown',
            'toast.saveFailed': '保存失败，请立即导出备份',
            'toast.storageInitFailed': '本地数据库初始化失败',
            'import.modePrompt': '输入 merge 合并笔记，或 replace 覆盖全部笔记（覆盖前会自动下载备份）',
            'import.invalidMode': '导入模式必须是 merge 或 replace',
            'import.mixedFiles': '请只选择 JSON、单个 ZIP，或一组 Markdown 文件',
        },
        'en': {
            'toolbar.backupJson': 'Export full JSON backup',
            'toolbar.exportMarkdown': 'Export Markdown ZIP',
            'toolbar.importNotes': 'Import JSON, ZIP, or Markdown',
            'toast.saveFailed': 'Save failed. Export a backup now.',
            'toast.storageInitFailed': 'Local database initialization failed',
            'import.modePrompt': 'Enter merge to add notes, or replace to replace all notes (a backup downloads first)',
            'import.invalidMode': 'Import mode must be merge or replace',
            'import.mixedFiles': 'Select one JSON, one ZIP, or a group of Markdown files',
        },
        'ja': {
            'toolbar.backupJson': 'JSON完全バックアップを出力',
            'toolbar.exportMarkdown': 'Markdown ZIPを出力',
            'toolbar.importNotes': 'JSON、ZIP、Markdownを読み込む',
            'toast.saveFailed': '保存に失敗しました。すぐにバックアップを出力してください。',
            'toast.storageInitFailed': 'ローカルデータベースの初期化に失敗しました',
            'import.modePrompt': '追加は merge、全置換は replace を入力してください（置換前にバックアップします）',
            'import.invalidMode': 'モードは merge または replace を指定してください',
            'import.mixedFiles': 'JSON 1件、ZIP 1件、またはMarkdownファイルのみを選択してください',
        },
    };
...
`,
    ...
    function t(key, params) {
        ...
        let str = (map && map[key])
            ? map[key]
            : (_storageTranslations[_lang]?.[key]
                || _translations['zh-CN'][key]
                || _storageTranslations['zh-CN'][key]
                || key);
        if (params) {
